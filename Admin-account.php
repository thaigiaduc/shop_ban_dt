...
<div class="row" style="width: 700px;height: auto; margin-left: 450px;margin-top:-100px; z-index: 3;position: fixed;display: none;" id="addsp">
  <div class="col-75">
    <div class="container">
      <form method="post">
        ...
        <div class="row">
          <div class="col-50">
            ...
            <input type="text" id="brand" name="password" placeholder="Password">
			<label for="brand">Họ đệm</label>
            <input type="text" id="brand" name="hodem" placeholder="Họ đệm">
			<label for="brand">Tên</label>
            <input type="text" id="brand" name="ten" placeholder="Tên">
            <label for="phienbang">Phân quyền</label>
            <input type="text" id="phienbang" name="phanquyen" placeholder="Quyền 1 (Nhân viên) - Quyền 2 (Admin)">			
          </div>          
          
        </div>        
        <input type="submit" name="nut" value="Thêm User" class="btn">
			  ...
			  case 'Thêm User':
				{
					...
					if($id!='' && $user!='' && $pass!='' && $quyen!='' && $hodem!='' && $ten!='')
					{
						$p->adduser($id,$user,$pass,$hodem,$ten,$quyen);
					}
					else
					{
...
<div id="xoa" style="width: 500px; margin-left: 500px; display: none;">
  <form id="form1" name="form1" method="post">
	  <input type="text" name="xoa" placeholder="Nhập ID cần xóa">
...
<div id="sua" style="width: 500px; margin-left: 500px; display: none;">
  <form id="form1" name="form1" method="post">
	  <input type="text" name="sua" placeholder="Nhập ID cần sửa">
	  ...
	  <?php
	  	switch($_POST['nut'])
		{
			case 'Sửa':
			{
				...
				if($id!='')
				{
					$p->edituser("select * from user where ID='$id'");
				}
				else
				{
	  ...
	  switch($_POST['nut'])
		{
			case 'Sửa user':
			{
				$id=$_POST['id'];
				$ten=$_POST['username'];
				$nhanhieu=$_POST['password'];
				$hodem = $_POST['hodem'];
				$tennv = $_POST['ten'];
				$phienbang=$_POST['phanquyen'];				
					if($id!='' && $ten!='' && $nhanhieu!='' && $phienbang!='')
					{
						$p->editacc($id,$ten,$nhanhieu,$hodem,$tennv,$phienbang);
					}
					else
					{
						echo '<script>alert("Bạn hãy nhập đầy đủ thông tin")</script>';
