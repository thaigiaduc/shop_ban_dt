      ...
      <form method="post">
        ...
        <div class="row">
          <div class="col-50">
            ...
            <input type="text" id="id" name="id" placeholder="ID khách hàng" value="<?php echo time() ?>">
        ...
        <input type="submit" name="nut" value="Nhập khách hàng" class="btn">
		<input type="button" value="Thoát" class="btn" onClick="thoat()">
		<?php
		  switch($_POST['nut'])
		  {
			  case 'Nhập khách hàng':
				{
					$id=$_POST['id'];
					$ten=$_POST['name'];
					...
					{
						$p->addkh($id,$ten,$nhanhieu,$phienbang);
					}
					else
					{
	  ...
	  <?php
	  	switch($_POST['nut'])
		{
			case 'Sửa':
			{
				...
				if($id!='')
				{
					$p->btneditkh("select * from khachhang where IDKH='$id'");
				}
				else
				{
	  ...
	  switch($_POST['nut'])
		{
			case 'Sửa KH':
			{
				...
				$phienbang=$_POST['sdt'];								
					if($id!='' && $ten!='' && $nhanhieu!='' && $phienbang!='')
					{
						$p->editkh($id,$ten,$nhanhieu,$phienbang);
					}
					else
					{
						echo '<script>alert("Bạn hãy nhập đầy đủ thông tin")</script>';
...
<div style="margin-top: -50px;">
		<table id="customers">
		  <tr>
			...
			<?php
				$p->tablekhachhang("select * from khachhang");
				switch($_POST['nut'])
				{
					case 'Xóa':
					{	
				...
				switch($_POST['nut'])
				{
					...
					{	
						$id=$_GET['id'];
						$p->editsuakh("select * from khachhang where IDKH='$id'");
						break;
					}
				}
			?>	  
